 ...
 */
private [niocomm] class CommandQueue(backlog : Int, notifier : () ⇒ Unit) {
  ...
  /** Last item index. */
  private[niocomm] val last = store.length - 1
  ...
  private[niocomm] def put(item : T) : Unit =
    lock synchronized {
      if (eof)
   ...
   */
  private[niocomm] def putKiller(item : T) : Boolean =
    lock synchronized {
      if (eof)
        return false
   ...
   */
  private[niocomm] def getCommand() : T = {
